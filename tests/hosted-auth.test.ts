...
import { createAuthGateway } from "../src/hosted/auth.js";

const methods = vi.hoisted(() => ({ signInWithOtp: vi.fn(), verifyOtp: vi.fn(), getUser: vi.fn(), signOut: vi.fn() }));
...
describe("GoTrue authentication boundary", () => {
  it("disables account creation and keeps invited/unknown email responses indistinguishable", async () => {
    ...
    expect(AuthClient).toHaveBeenLastCalledWith({ url: "http://auth:9999", autoRefreshToken: false, persistSession: false, detectSessionInUrl: false, headers: { "x-capykit-auth-client": "capykit-api" } });
  });
  ...
  it("uses server-verified user identity for every explicit token, never cached SDK identity", async () => {
    ...
    await expect(gateway.verifyBearer("verified-token")).resolves.toEqual({ provider: "gotrue", subject: "owner-1", email: "owner@example.test" });
  ...
  it("ends only the presented provider session without an admin credential and tolerates already-invalid sessions", async () => {
    const gateway = createAuthGateway(config);
    if (gateway === undefined) throw new Error("test auth configuration missing");
    methods.signOut.mockResolvedValueOnce({ error: null });
    await expect(gateway.signOut("verified-token")).resolves.toBeUndefined();
    expect(methods.signOut).toHaveBeenCalledWith("verified-token", "local");
    methods.signOut.mockResolvedValueOnce({ error: { status: 403, message: "session no longer exists" } });
    await expect(gateway.signOut("revoked-token")).resolves.toBeUndefined();
    methods.signOut.mockResolvedValueOnce({ error: { status: 500, message: "SENTINEL_PROVIDER_DETAIL" } });
    await expect(gateway.signOut("verified-token")).rejects.toThrow("Authentication provider unavailable");
  });
});
